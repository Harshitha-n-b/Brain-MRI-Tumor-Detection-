...
import { HeartPulse, Mail, Lock, User, AlertCircle, ArrowRight, Loader2 } from 'lucide-react';
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-950 px-6 py-12 relative overflow-hidden transition-colors duration-300">
      ...
      >
        <div className="flex flex-col items-center gap-2 mb-8">
          ...
          <h2 className="text-2xl font-bold text-slate-900 dark:text-white">Create Workstation Account</h2>
          <p className="text-sm text-slate-500 dark:text-slate-400 text-center">
            Register your clinical credentials to begin MRI evaluations
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4.5">
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-semibold text-slate-500 dark:text-slate-400">Full Name / MD Name</label>
            <div className="relative">
              ...
              <input 
                ...
                className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl pl-11 pr-4 py-3 text-sm focus:outline-none focus:border-primary-500 focus:bg-white transition-all text-slate-900 dark:text-white"
              />
            </div>
          </div>
          ...
          >
            {loading ? (
              <>
                <Loader2 className="w-4.5 h-4.5 animate-spin" /> Registering Account...
              </>
            ) : (
              <>
                Register Workstation <ArrowRight className="w-4 h-4" />
              </>
            )}
          </button>
        </form>
        ...
        <div className="mt-8 text-center text-sm text-slate-550 dark:text-slate-400">
          Already registered?{' '}
          <Link to="/login" className="text-primary-550 dark:text-primary-400 font-bold hover:underline">
            Log In
          </Link>
        </div>
      </motion.div>
